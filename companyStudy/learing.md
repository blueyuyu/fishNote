...
## ant-design table 简单操作

```
          <a-table :columns="columns" :data-source="dataSource" :pagination="false" :loading="loading" size="small">
            <template #bodyCell="{ column, record }">
              <template v-if="column.key === 'action'">
                <span>
                  ...
                  <a>挂起</a>
                  <a-divider type="vertical" />
                  <a class="ant-dropdown-link"> 终止 </a>
                </span>
              </template>
            </template>
          </a-table>
...
$eventBus.on('goMenu', handleGoMenuEvent);
$eventBus.emit('goMenu', sourceKey);

$eventBus.off('goMenu', handleGoMenuEvent);
const handleGoMenuEvent = (source: string) => {
console.log('Received goMenu event with source:', source);
console.log('menusRef.value:', menusRef.value);
// console.log('tagblue handleGoMenuEvent', 'source', source, 'menusRef', menusRef.value);
if (menusRef.value.length > 0) {
menus.length = 0; // Clear existing menus
if (source !== 'OTHER') {
menus.push(...menusRef.value.filter((item) => item?.source == source));
} else {
menus.push(...menusRef.value.filter((item) => item?.source == ''));
}
console.log('menus', menus);
} else {
console.log('menusRef is empty, waiting for it to be populated...');
}
};
```
